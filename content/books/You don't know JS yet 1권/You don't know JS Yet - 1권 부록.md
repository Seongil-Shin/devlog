...
- 또한 추론된 이름은 어디까지나 메타데이터일뿐이며 실제 해당 함수를 참조하는 식별자가 아니다. 따라서 익명 함수는 본문 안에서 자신을 참조할 수 있는 식별자를 갖지 못한다. 이는 재귀를 사용하거나, 등록된 이벤트를 해제할때 문제가 된다. (정확히 어떻게?)
...
대부분 익명함수표현식을 더 짧기 때문에 많이 사용하지만 함수에 의미있는 이름을 붙이는 것이 가독성이 훨씬 더 좋아지며 에러가 발생했을시 스택트레이스를 따라가기 좋다.

...
- 스코프 시작부분에 식별자가 function 선언과 함께 등장했으면 해당 변수는 연관된 함수 참조로 자동 초기화된다 `function someFunction() {...}` 
...
- **변수가 소스 역할을 할때 식별자를 찾지 못하면 해당 변수는 선언되지 않은 변수로 간주**되어 `ReferenceError`를 발생 시킨다. 
- **변수가 타깃 역할을 하고 프로그램이 엄격모드일때 해당 변수는 선언되지 않은 변수로 간주**되어 `ReferenceError`가 발생한다. 

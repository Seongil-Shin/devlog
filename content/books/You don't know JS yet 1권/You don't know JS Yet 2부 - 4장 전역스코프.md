...
위 코드를 브라우저에 불러오면 `studentName`, `hello`가 전역 스코프에 선언된다. 즉, 전역 객체 window의 프로퍼티를 통해 해당 식별자에 접근할 수 있다
...
- DOM 요소에 id 속성을 추가하면 전역변수가 자동으로 생긴다.
...
개발자 도구는 JS 환경을 완전히 재현하지 않는다. 일반 JS 프로그램보다 덜 엄격하게 처리하는데, 따라서 일반 JS 프로그램에서는 오류가 났을 코드가 개발자 도구에서는 정상동작하는 경우가 있다.

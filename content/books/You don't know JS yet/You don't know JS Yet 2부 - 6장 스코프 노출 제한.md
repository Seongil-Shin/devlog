...
- 화살표함수가 아닌 일반 함수 형태로 IIFE를 사용하면 this의 바인딩이 바뀐다.
- break, continue는 함수 경계를 넘지 않기에 제어문 안의 IIFE에서 break, continue를 사용한다고 IIFE 밖의 제어문을 통제할 수 있지 않다.
...
- let, const를 포함하지 않는 블록은 스코프를 만들지 않음.
...
하지만 이러한 FiB 패턴은 환경에따라 동작이 다르기에 사용하지 않는 것이 좋다. 함수 선언은 항상 함수의 최상위 스코프에서 하라.

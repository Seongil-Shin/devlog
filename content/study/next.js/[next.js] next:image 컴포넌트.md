...
이미지가 퍼포먼스에 악영향을 미치는 가장 큰 요인은 [cumulative layout shift](https://web.dev/cls/)이다 . 이미지가 늦게 로딩되어 다른 엘리먼트를 밀어내는 것을 말한다. 유저의 입장에선 매우 짜증나는 이슈이다. 

이 이슈를 막기 위해서는 [이미지의 사이즈를 기입](https://web.dev/optimize-cls/#images-without-dimensions)해야한다. 이미지의 사이즈를 기입하면, 브라우저는 이미지가 로드되기 전에 이미지를 위한 공간을 미리 잡아둔다.

`next/image`에서는 다음 세가지 방법중 하나를 사용한다.

1. `static import`를 통해 로컬에서 이미지를 가져오면 자동으로 계산한다.
...
### `fill` 속성 사용시, 부모요소는 `display: block`여야한다.
...
보통 Remote 에서 이미지를 가져오는 경우가 많으므로 Remote 기준으로 실험함
- width, height 가 이미지 크기에 영향을 주나 : css 설정이 되어있다면 css가 우선으로 적용됨. 크게 걱정하지 않아도 될듯
- lazy 로딩 적용됨 (img 태그의 `loading="lazy"`)
- 주소가 next.js 서버 주소로 변경됨. 이미지를 가져와서 next.js 서버에서 webp 등으로 가공한 후 내보냄
	- "Remote -> CDN" vs "Remote -> next.js 서버" 중 어느게 빠를지를 확인해봐야할듯. 후자의 경우 webp로 가공하기에 절대적인 이미지 용량이 적지만 확인은 필요
	- 가공한 이미지는 `/.next/cache/images` 아래에 저장되어 재사용한다. vercel 에 배포할 경우 최대 31일까지 저장해둠. 
...
https://kyoung-jnn.com/posts/nextjs-image-optimization

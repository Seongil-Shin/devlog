...
- Duration : 따로 revalidate하거나 opt-out하지 않는한 계속 유지된다.
- Revalidating
...
1. React Rendering on the Server
   - next.js는 React API를 사용하여 렌더링을 오케스트레이션한다. 이때 각 route segments와 Suspense boundaries를 chunks로 나눈다.
...
1. Next.js Caching on the Server (Full Route Server)
	 - 기본적으로 next.js는 렌더링 결과 (React Server Component Payload, HTML)을 캐싱한다. 빌드타임 또는 revalidation 시 생성되는 결과 캐싱
2. React Hydration and Reconciliation on the Client : 요청 시 클라이언트에서의 동작
	1. HTML 렌더링. 상호작용 불가능
...
- Invalidations 
	- Revalidating Data : Data Cache를 revalidating하면 컴포넌트를 서버에서 리렌더링하고, 새로운 렌더링 결과를 캐싱하여 Router Cache도 invalidation 된다.
...
- Invalidation
	-  [Server Action](https://nextjs.org/docs/app/building-your-application/data-fetching/server-actions)에서
		-  On-demand revalidating 사용 : `revalidatePath`, `revalidateTag`

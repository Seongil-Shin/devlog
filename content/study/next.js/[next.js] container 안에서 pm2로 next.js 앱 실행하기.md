...
Dockerfile에서 마지막 WORKDIR은 `/home/deploy/next`이므로, 명령어를 실행한 위치는 `/home/deploy/next`이다. 따라서 `home/deploy/next/.next`로 빌드파일의 경로를 설정해야하나, `ecosystem.config.js`로 경로를 설정한 것은 pm2의 문제가 있다고 생각하여 찾아보았다.
